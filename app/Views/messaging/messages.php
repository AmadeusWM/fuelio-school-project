        ...
        <?php foreach ($notifications as $notification) { ?>
            <li class="notification">
                <span class="sender-name">by
                    <a href="<?= base_url("/message") . "/" . esc($notification["sender_id"]) ?>" tabindex="-1">
                        <?= esc($notification["sender_username"]) ?>
                    </a>
                    <?php
                    if ($notification["sender_webshop"] != "") { ?>
                        from <a href="<?= base_url("/store/webshop") . "/" . esc($notification["sender_id"]) ?>" tabindex="-1">
                ...
                <?php if ($notification["type"] == "message"){ ?>
                    <a class="btn btn-primary" href="<?= base_url("/message") . "/" . esc($notification["sender_id"]) ?>" tabindex="-1"><i class="bi bi-envelope" aria-label="Send Response"></i></a>
                <?php } else{ ?>
                    <a class="btn btn-primary" href="<?= base_url("/store/product") . "/" . esc($notification["pointer_id"]) ?>" tabindex="-1"><i class="bi bi-arrow-return-right" aria-label="Visit Product"></i></a>
            <?php } ?>
                <hr />
            </li>
        <?php } ?>
